...
app.get('/quiz/questions', async (req, res) => {
    ...
    try {
        ...
        const questionResult = await client.query(
            `SELECT id, question_text, option_a, option_b, option_c, option_d,correct_answer AS correct_option,explanation,image_url
             FROM questions
             WHERE topic_id = $1`,
            [topicId]
        );
...
app.get('/admin/question-analysis', async (req, res) => {
    ...
    if (!topicId || !schoolId) {
        return res.status(400).json({ success: false, message: 'Missing topicId or schoolId' });
    }
...
app.get('/admin/available-topics', async (req, res) => {
  ...
  try {
    const result = await pool.query(
      `SELECT t.id, t.name
       FROM topics t
       JOIN school_curriculum_topics sct ON sct.topic_id = t.id
       WHERE sct.school_id = $1 AND t.chapter_id = $2`,
      [schoolId, chapterId]
    );
...
app.get('/admin/available-chapters', async (req, res) => {
  const { schoolId, className, subjectId } = req.query;
  ...
  try {
    ...
    console.error('Error fetching available chapters:', err);
...
app.get('/superadmin/quiz-summary', async (req, res) => {
  ...
  try {
    // Fetch only subjects assigned to the school and class
    ...
    const result = await Promise.all(subjects.map(async (subject) => {
      // Assigned quizzes (count DISTINCT topic_id to avoid duplicates from multiple sections)
      const assignedCountResult = await pool.query(
        `SELECT COUNT(DISTINCT qa.topic_id) FROM quiz_assignments qa
         JOIN topics t ON qa.topic_id = t.id
         WHERE qa.school_id = $1 AND t.class = $2 AND t.subject_id = $3`,
        [schoolId, className, subject.id]
      );

      // Total quizzes available from curriculum assignment
